...
import { fileType, folderType, workspaceType } from "@/src/lib/supabase/types";
import { useAppState } from "@/src/provider/state-provider";
import { usePathname, useRouter } from "next/navigation";
import { TOOLBAR_OPTIONS } from "@/src/lib/utils";
...
interface QuillEditorProps {
  dirDetails: fileType | folderType | workspaceType;
  fileId: string;
...
export const QuillEditor = ({
  ...
  const details = useMemo(() => {
    ...
    } as workspaceType | folderType | fileType;
  ...
  }, [quill, socket, fileId, user, details, dirType, folderId, workspaceId, dispatch]);
      ...
      .subscribe(async (status) => {
        ...
        room.track({
          ...
          avatarUrl: data.image
            ? supabase.storage.from("avatars").getPublicUrl(data.image).data
              .publicUrl
            : "",
        });
      });
              ...
              {collaborators?.map((collaborator) => (
                <TooltipProvider key={collaborator.id}>
                  ...
                  "
                      >
                        <AvatarImage
                          src={collaborator.image ? collaborator.image : ""}
